...
class CreateCuscKhuyenmaiSanphamTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('cusc_khuyenmai_sanpham', function (Blueprint $table) {
            ...
            $table->foreign('km_ma')->references('km_ma')->on('cusc_khuyenmai');
            $table->foreign('m_ma')->references('m_ma')->on('cusc_mau');
            $table->foreign('sp_ma')->references('sp_ma')->on('cusc_sanpham');
        });
