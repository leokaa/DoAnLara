...
class CreateCuscHoadonsiTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('cusc_hoadonsi', function (Blueprint $table) {
            ...
            $table->foreign('dh_ma')->references('dh_ma')->on('cusc_donhang')->onDelete('CASCADE');
            $table->foreign('nv_lapHoaDon')->references('nv_ma')->on('cusc_nhanvien')->onDelete('CASCADE');
            $table->foreign('nv_thuTruong')->references('nv_ma')->on('cusc_nhanvien')->onDelete('CASCADE');
            $table->foreign('tt_ma')->references('tt_ma')->on('cusc_thanhtoan')->onDelete('CASCADE');
        });
