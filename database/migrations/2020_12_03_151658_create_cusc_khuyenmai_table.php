...
class CreateCuscKhuyenmaiTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('cusc_khuyenmai', function (Blueprint $table) {
            ...
            $table->unique(['km_ten']);
            $table->foreign('nv_kyDuyet')->references('nv_ma')->on('cusc_nhanvien');
            $table->foreign('nv_kyNhay')->references('nv_ma')->on('cusc_nhanvien');
            $table->foreign('nv_nguoiLap')->references('nv_ma')->on('cusc_nhanvien');
        });
